...
def parse_one_json(json_dir,speaker_list,f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13,f14,f15,intype):
	...
	for i in range(args.len):
		step_pref_all.append(args.len-i)
		i_pref_all.append("None-none")
		s_pref_all.append('O')
		nl_pref_all.append(empty)
	for line in data["utterances"]:
		...
		for i in range(len(line["semantic_tagged"])):
			...
			nl_pref_all = nl_pref_all[1:]  
			i_pref_all = i_pref_all[1:]
			s_pref_all = s_pref_all[1:]
			step_pref_all = step_pref_all[1:]
			nl_pref_all.append(nl_pref)
			i_pref_all.append(i_pref)
			s_pref_all.append(s_pref)
			step_pref_all.append(0)
			for n in range(len(step_pref_all)):
				step_pref_all[n] = step_pref_all[n] + 1
